function Game() {
	...
	function updateBoardUI() {
		for (let i = 0; i < square.length; i++) {
			let cell = document.getElementById("cell" + i + "name");
			if (cell) {
				cell.innerHTML = square[i].image ? `<img src="${square[i].image}" width="50px">` : square[i].name;
			}
		}
	}
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert(p.name + " bought " + sq.name + " for ATP" + highestbid + ".");
		}
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						this.auctionBid(bid);
						window.alert(p.name + " bid ATP" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			...
			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "You don't have enough money to bid ATP" + bid + ".";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Your bid must be greater than highest bid. (ATP" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		for (var i = 0; i < 34; i++) {
			...
			if (currentSquare.owner === initiator.index) {
				...
				if (currentSquare.groupNumber === 0) {
		...
		if (initiator.specialstationJailCard) {
			currentTableRow = initiatorSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
			...
			currentTableCellCheckbox.title = "Check this box to include this Get Out of Enzyme Inhibition Zone Free Card in the trade.";
			...
			currentTableCell.textContent = "Get Out of Enzyme Inhibition Zone Free Card";
		} else if (recipient.specialstationJailCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
	...
	var readTrade = function() {
		...
		var trade = new Trade(initiator, recipient, money, property, specialstationJailCard, chanceJailCard);
	...
	var writeTrade = function(tradeObj) {
		...
		if (document.getElementById("tradeleftcheckbox40")) {
			if (tradeObj.getspecialstationJailCard() === 1) {
	...
	this.acceptTrade = function(tradeObj) {
		...
		if (money > 0 && money > initiator.money) {
			document.getElementById("trade-leftp-money").value = initiator.name + " does not have ATP" + money + ".";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		} else if (money < 0 && -money > recipient.money) {
			document.getElementById("trade-rightp-money").value = recipient.name + " does not have ATP" + (-money) + ".";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		isAPropertySelected |= tradeObj.getspecialstationJailCard();
		...
		if (tradeObj.getspecialstationJailCard() === 1) {
			initiator.specialstationJailCard = false;
			recipient.specialstationJailCard = true;
			addAlert(recipient.name + ' received a "Get Out of Enzyme Inhibition Zone Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getspecialstationJailCard() === -1) {
			initiator.specialstationJailCard = true;
			recipient.specialstationJailCard = false;
			addAlert(initiator.name + ' received a "Get Out of Enzyme Inhibition Zone Free" card from ' + recipient.name + ".");
		}
		...
		if (money > 0) {
			...
			addAlert(recipient.name + " received ATP" + money + " from " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert(initiator.name + " received ATP" + money + " from " + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		var reversedTrade = new Trade(recipient, initiator, -money, reversedTradeProperty, -tradeObj.getspecialstationJailCard(), -tradeObj.getChanceJailCard());
	...
	this.bankruptcy = function() {
		...
		if (p.specialstationJailCard) {
			p.specialstationJailCard = false;
			pcredit.specialstationJailCard = true;
		}

		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			game.eliminatePlayer();
		} else {
			addAlert(pcredit.name + " paid ATP" + bankruptcyUnmortgageFee + " interest on the mortgaged properties received from " + p.name + ".");
			popup("<p>" + pcredit.name + ", you must pay ATP" + bankruptcyUnmortgageFee + " interest on the mortgaged properties you received from " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
	updateBoardUI();
}
...
function Player(name, color) {
	...
	this.jail = false;
	this.jailroll = 0;
	this.specialstationJailCard = false;
...
// specialstationJailCard: integer, 1 means offered, -1 means requested, 0 means neither
// chanceJailCard: integer, 1 means offered, -1 means requested, 0 means neither
function Trade(initiator, recipient, money, property, specialstationJailCard, chanceJailCard) {
	// For each property and get out of Enzyme Inhibition Zone free cards, 1 means offered, -1 means requested, 0 means neither.
	...
	this.getspecialstationJailCard = function() {
		return specialstationJailCard;
	};
...
function updatePosition() {
	...
	for (var i = 0; i < 34; i++) {
		document.getElementById("cell" + i).style.border = "1px solid #7c5c36";
	...
	for (var i = 1; i < turn; i++) {
		if (player[i].jail) {
			document.getElementById("jailpositionholder").innerHTML += "<div class='cell-position' title='" + player[i].name + "' style='background-color: " + player[i].color + "; left: " + left + "px; top: " + top + "px;'></div>";			if (left === 34) {
				left = 0;
				top = 41;
			} else
				left += 12;
...
function updateATP() {
	...
	document.getElementById("pmoney").innerHTML = "ATP" + p.money;
	$(".money-bar-row").hide();
...
function updateOwned() {
	...
	if (p.specialstationJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Enzyme Inhibition Zone Free Card</td></tr>";
	...
	if (p.chanceJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Enzyme Inhibition Zone Free Card</td></tr>";
	}
...
function updateOption() {
	...
	if (sq.mortgage) {
		document.getElementById("mortgagebutton").value = "Unmortgage (ATP" + Math.round(sq.price * 0.55) + ")";
		document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " for ATP" + Math.round(sq.price * 0.55) + ".";
		$("#buyhousebutton").hide();
		$("#sellhousebutton").hide();

		allGroupUnmortgaged = false;
	} else {
		document.getElementById("mortgagebutton").value = "Mortgage (ATP" + (sq.price * 0.5) + ")";
		document.getElementById("mortgagebutton").title = "Mortgage " + sq.name + " for ATP" + (sq.price * 0.5) + ".";
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Buy house (ATP" + sq.houseprice + ")";
			sellhousebutton.value = "Sell house (ATP" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Buy a house for ATP" + sq.houseprice;
			sellhousebutton.title = "Sell a house for ATP" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Buy hotel (ATP" + sq.houseprice + ")";
				buyhousebutton.title = "Buy a hotel for ATP" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Sell hotel (ATP" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Sell a hotel for ATP" + (sq.houseprice * 0.5);
			}
...
function landspecialstation() {
    var p = player[turn];

    // Check if the player landed on a special station
    if (p.position === 2 || p.position === 4 || p.position === 6 || p.position === 23) {
        var cardIndex = specialCards.index;  // Store current card index
        let drawnCard = specialCards[cardIndex];  // Get the drawn card

    // Validate the drawn card
	if (!drawnCard || typeof drawnCard.action !== "function") {
        console.error("Error: Invalid or missing action for special card.");
        return;
    }

    console.log(`🎴 Special Card Drawn: ${drawnCard.name}`);
    alert(`Special Card: ${drawnCard.name}\n\n${drawnCard.text}`);

    // Apply the effect
    drawnCard.action(p);
    console.log(`✅ Effect applied to ${p.name}: ${drawnCard.name}`);

    // Update ATP and UI
	updateATP();
	updateBoardUI();
		
			popup("<img src='images/special-station-card.png' style='height: 425px; width: 250px; margin: 8px 8px 8px 8px; display: block; ' /> <div style='font-weight: bold; font-size: 16px;'>Special Card:</div><div style='text-align: center; font-size: 16px; position: absolute;top: 40%; left: 48%; transform: translate(-50%, -50%); margin: 8px 8px 8px 8px; display: block;'>" + drawnCard.text + "</div>", function() {
                drawSpecialCard(); // Apply the card effect immediately
            }
        );

        // Move to the next special card
        specialCards.index++;

        // Reset index if we reach the end of the deck
        if (specialCards.index >= specialCards.length) {
            specialCards.index = 0;
        }

	// Enzyme Cards
	} else if (p.position === 1 || p.position === 3 || p.position === 5 
        || p.position === 7 || p.position === 8 || p.position === 9 
        || p.position === 11 || p.position === 12 || p.position === 13 || p.position === 14 
		|| p.position === 15 || p.position === 16 || p.position === 18 || p.position === 19
        || p.position === 20|| p.position === 21 || p.position === 22 || p.position === 24 
        || p.position === 25|| p.position === 26 || p.position === 28 || p.position === 29 
        || p.position === 30 || p.position === 31 || p.position === 32 || p.position === 33) {
		var enzymeCardsIndex = enzymeCards.deck[enzymeCards.index];

		// Remove the get out of Enzyme Inhibition Zone free card from the deck.
		if (enzymeCardsIndex === 0) {
			enzymeCards.deck.splice(enzymeCards.index, 1);
		}

		popup("<img src='images/enzyme-card.png' style='height: 425px; width: 250px; margin: 8px 8px 8px 8px; display: block; ' /><div style='font-weight: bold; font-size: 16px;'>Cofactor Card:</div><div style='text-align: center; font-size: 16px; position: absolute;top: 40%; left: 48%; transform: translate(-50%, -50%); margin: 8px 8px 8px 8px; display: block;'>" + enzymeCards[enzymeCardsIndex].text + "</div>", function() {
			drawEnzymeCard(p, cardIndex);
		});

		enzymeCards.index++;

		if (enzymeCards.index >= enzymeCards.deck.length) {
			enzymeCards.index = 0;
		}
	} else {
		if (!p.human) {
...
function drawEnzymeCard(p, cardIndex) {
    let card = enzymeCards[Math.floor(Math.random() * enzymeCards.deck.length)];
    let validStations = [];
	var p = player[turn];

    // Define the enzyme card indices and corresponding valid station positions
    let enzymeStationMap = {
        0: [33], 1: [1], // CoASH
        2: [33], 3: [33], // ADP
        4: [26, 23, 24, 11, 27, 28, 29, 34, 35, 13], 5: [26, 23, 24, 11, 27, 28, 29, 34, 35, 13], // Mg 2+
        6: [13], 7: [13], // Biotin
        8: [12], 9: [12], // GTP
        10: [13], 11: [13], // CO2
        12: [9], 13: [9], // cAMP
        14: [1], 15: [1], // FAD
        16: [22], 17: [22], // UDP Glucose
        18: [23], 19: [23], // Pi
        20: [1], 21: [1], // NAD+
        22: [28], 23: [28], // PARP-1
        24: [1], 25: [1], // TPP
        26: [7], 27: [7], // NADPH
        28: [5], 29: [5], // Transcription factors
        30: [33, 23], 31: [33, 23], // Ca2+
        32: [1], 33: [1], // Lipoic acid
        34: [16], 35: [16]  // SAM
    };

    // Get the valid stations for this enzyme card
    if (enzymeStationMap[enzymeCards.indexOf(card)]) {
        validStations = enzymeStationMap[enzymeCards.indexOf(card)];
    }

    if (validStations.includes(p.position)) {
        applyCardEffect(cardIndex, player);
        alert(`${card.text} effect applied!`);
    } else {
        alert("Cofactor doesn't match the enzyme station.");
    }
}

function applyCardEffect(cardIndex, player) {
    switch (cardIndex) {

		case 0: // CoASH (Pyruvate dehydrogenase, Station 33 and a-ketoglutarate dehydrogenase complex, Station 1)
			alert("CoASH effect applied! Skip the next rent.");
			player.skipNextRent = true;
		break;

		case 1: // CoASH Negative Effect
			alert("CoASH negative effect! Lose one of your properties.");
			loseOneProperty(player);
		break;

		case 2: // ADP (Pyruvate dehydrogenase, Station 33)
			alert("ADP effect applied! Move forward 2 spaces.");
			movePlayerForward(player, 2);
		break;

		case 3: // ADP Negative Effect
			alert("ADP negative effect! Move back 2 spaces.");
			movePlayerBack(player, 2);
		break;

		case 4: // Mg2+ (Hexokinase, Station 25 and Phosphoglycerate kinase, Station 23 and Pyruvate kinase, Station 24 and G6P, Station 11 and DNA polymerase, Stations 27-35, and Pyruvate Carboxylase, Station 13)
			alert("Mg2+ effect applied! Move forward 1 space.");
			movePlayerForward(player, 1);
		break;

		case 5: // Mg2+ Negative Effect
			alert("Mg2+ negative effect! Move back 1 space.");
			movePlayerBack(player, 1);
		break;

		case 6: // Biotin (Pyruvate Carboxylase, Station 13)
			alert("Biotin effect applied! Roll the dice again.");
			rollDiceAgain(player, 1);
		break;

		case 7: // Biotin Negative Effect
			alert("Biotin negative effect! Freeze for the next 2 turns.");
			player.freezeTurns = 2;
		break;

		case 8: // GTP (PEPCK, Station 12)
			alert("GTP effect applied! Double your rent for 1 round.");
			player.doubleNextRent = true;
		break;

		case 9: // GTP Negative Effect
			alert("GTP negative effect! Lose 20 ATP points.");
			player.atp -= 20;
            updateATP(player);
		break;

		case 10: // CO2 (Pyruvate Carboxylase, Station 13)
			alert("CO2 effect applied! Skip the next rent.");
			player.skipNextRent = true;
		break;

		case 11: // CO2 Negative Effect
			alert("CO2 negative effect! Move back 4 spaces.");
			movePlayerBack(player, 4);
		break;

		case 12: // cAMP (Hormone-Sensitive Lipase (HSL), Station 9)
			alert("cAMP effect applied! Gain 20 ATP points.");
			player.atp += 20;
            updateATP(player);
		break;

		case 13: // cAMP Negative Effect
			alert("cAMP negative effect! Go to Enzyme Inhibition Zone.");
			sendToInhibitionZone(player);
		break;

		case 14: // FAD (a-ketoglutarate dehydrogenase complex, Station 1)
			alert("FAD effect applied! Move forward 4 spaces.");
			movePlayerForward(player, 4);
		break;

		case 15: // FAD Negative Effect
			alert("FAD negative effect! Freeze for the next turn.");
			player.freezeTurns = 1;
		break;

		case 16: // UDP Glucose (Glycogen synthase, Station 22)
			alert("UDP Glucose effect applied! Roll the dice again.");
			rollDiceAgain(player, 1);
		break;

		case 17: // UDP Glucose Negative Effect
			alert("UDP Glucose negative effect! Go to Enzyme Inhibition Zone.");
			sendToInhibitionZone(player);
		break;

		case 18: // Pi (Glycogen Phosphorylase, Station 23)
            alert("Pi effect applied! Draw an additional enzyme card.");
            drawEnzymeCard(player);
            break;

        case 19: // Pi Negative Effect
            alert("Pi negative effect! Lose 5 ATP.");
            player.atp -= 5;
            updateATP(player);
            break;

		case 20: // NAD+ (a-KG complex, Station 1)
            alert("NAD+ effect applied! Collect 10 ATP from each player.");
            collectFromEachPlayer(player, 10);
        break;
        
        case 21: // NAD+ (Negative Effect)
            alert("NAD+ negative effect! Pay 15 ATP to the previous player.");
            payToPreviousPlayer(player, 15);
            break;

		case 22: // PARP-1 (DNA polymerase beta, Station 28)
            alert("PARP-1 effect applied! Draw 2 cards on your next enzyme station.");
            player.drawTwoNextStation = true;
        break;

		case 23: // PARP-1 (Negative Effect)
            alert("PARP-1 negative effect! Pay 15 ATP to the next player.");
			payToNextPlayer(player, 15);
        break;

		case 24: // TPP (a-KG complex, Station 1)
            alert("TPP effect applied! Gain 10 ATP points.");
			player.atp += 10;
            updateATP(player);
        break;

		case 25: // TPP (Negative Effect)
            alert("TPP negative effect! Can't buy your next enzyme station.");

        break;

		case 26: // NADPH (Billverdin reductase, Station 7)
            alert("NADPH effect applied! Collect 10 ATP from each player.");
			collectFromEachPlayer(player, 10);
        break;

		case 27: // NADPH (Negative Effect)
            alert("NADPH negative effect! Lose 10 ATP points.");
			player.atp -= 10;
            updateATP(player);
        break;
			
        case 28: // Transcription Factors (RNA Polymerase, Station 5)
            alert("Transcription Factors effect applied! Draw an additional enzyme card.");
            drawEnzymeCard(player);
            break;

		case 29: // Transcription Factors (Negative Effect)
            alert("Transcription Factors negative effect! Freeze for the next 2 turns.");
			player.freezeTurns = 2;
        break;

        case 30: // Ca2+ (Pyruvate Dehydrogenase & Glycogen Phosphorylase)
            alert("Ca2+ effect applied! Draw two cards on your next enzyme station.");
            player.drawTwoNextStation = true;
            break;

        case 31: // Ca2+ Negative Effect
            alert("Ca2+ negative effect! You won't receive ATP next round.");
            player.skipNextATP = true;
            break;

        case 32: // Lipoic Acid (a-KG complex, Station 1)
            alert("Lipoic Acid effect! Collect rent of your current station from each player.");
			collectStationRentFromEachPlayer(player);
            break;

        case 33: // Lipoic Acid Negative Effect
            alert("Lipoic Acid negative effect! Move back 1 space.");
            movePlayerBack(player, 1);
            break;

        case 34: // SAM (Methyl Transferase, Station 16)
            alert("SAM effect! Roll the dice again twice.");
            rollDiceAgain(player, 2);
            break;

        case 35: // SAM Negative Effect
            alert("SAM negative effect! Freeze for the next 3 turns.");
            player.freezeTurns = 3;
            break;
...
function movePlayerBack(player, spaces) {
    player.position = Math.max(0, player.position - spaces);
    updatePosition(player);
	updateBoardUI();
}

// Moves a player forward a number of spaces
function movePlayerForward(player, spaces) {
    player.position = (player.position + spaces) % square.length;
    updatePosition(player);
	updateBoardUI();
}

function collectFromEachPlayer(player, amount, pcount) {
    pcount.forEach(p => {
        if (p !== player) {
            p.atp -= amount;
            player.atp += amount;
...
// Pays ATP to the previous player
function payToPreviousPlayer(player, amount) {
    let previousPlayer = pcount[(player[turn] - 1 + pcount.length) % pcount.length];
    player.atp -= amount;
    previousPlayer.atp += amount;
    updateATP(player);
    updateATP(previousPlayer);
}

// Pays ATP to the next player
function payToNextPlayer(player, amount) {
    let nextPlayer = pcount[(player[turn] + 1) % pcount.length];
    player.atp -= amount;
    nextPlayer.atp += amount;
    updateATP(player);
    updateATP(nextPlayer);
}

// Moves the player to the Enzyme Inhibition Zone (Station 10)
function sendToInhibitionZone(player) {
    alert(player.name + " has been sent to the Enzyme Inhibition Zone!");
    player.position = 10;
    updatePosition(player);
}

// Player loses one property randomly
function loseOneProperty(player) {
    if (player.properties.length > 0) {
        let lostProperty = player.properties.splice(Math.floor(Math.random() * player.properties.length), 1)[0];
        lostProperty.owner = null; // Unowned again
        alert(player.name + " lost " + lostProperty.name + "!");
    } else {
        alert("No properties to lose.");
    }
}

// Player collects rent from all players for their current station
function collectStationRentFromEachPlayer(player) {
    let station = square[player.position];

    if (station.owner !== player) {
        alert("This station is not owned by you.");
        return;
    }

    let rentAmount = station.rent[0]; // Base rent
    pcount.forEach(p => {
        if (p !== player) {
            p.atp -= rentAmount;
            player.atp += rentAmount;
        }
    });
    updateATP(player);
}

// Rolls the dice again for a given number of times
function rollDiceAgain(player, times, rollDice) {
    for (let i = 0; i < times; i++) {
        let roll = rollDice();
        player.position = (player.position + roll) % square.length;
    }
    updatePosition(player);
}

function chanceAction(enzymeCardsIndex) {
	var p = player[turn]; // This is needed for reference in action() method.

	// $('#popupbackground').hide();
	// $('#popupwrap').hide();
	enzymeCards[enzymeCardsIndex].action(p);

	updateATP();

	if (enzymeCardsIndex !== 35 && !p.human) {
		p.AI.alertList = "";
		game.next();
	}
}
...
// Special Cards
function applySpecialCardEffect(cardIndex, player) {
    if (!player) {
        console.error("Error: Player reference is missing!");
        return;
    }

    switch (cardIndex) {
        case 0: // Insulin Stimulation
            alert("Insulin Stimulation! Gain 25 ATP and draw another special card.");
            player.atp += 25;
            updateATP(player);
            setTimeout(() => drawSpecialCard(), 500); // Prevents recursive call from breaking game
            break;

        case 1: // Insulin Inhibition
            alert("Insulin Inhibition! Miss your next turn and lose 15 ATP.");
            player.missNextTurn = true;
            player.atp -= 15;
            updateATP(player);
            break;

        case 2: // Glucagon Inhibition
            alert("Glucagon Inhibition! Reversing the effect of your last turn.");
            reverseLastEffect(player);
            break;

        case 3: // Glucagon Stimulation
            alert("Glucagon Stimulation! Combine two enzyme stations and double ATP for 3 turns.");
            player.combineStations = true;
            player.doublePathwayATP = 3;
            break;

        case 4: // Increased Metabolic Rate
            alert("Increased Metabolic Rate! Double ATP rent for 3 turns.");
            player.doubleRentTurns = 3;
            break;

        case 5: // Feedback Inhibition
            alert("Feedback Inhibition! You cannot draw enzyme cards for 2 turns.");
            player.noDrawTurns = 2;
            break;

        case 6: // Epinephrine Stimulation
            alert("Epinephrine Stimulation! Instantly gain 10 ATP.");
            player.atp += 10;
            updateATP(player);
            break;

        case 7: // Epinephrine Inhibition
            alert("Epinephrine Inhibition! Lose 15 ATP instantly.");
            player.atp -= 15;
            updateATP(player);
            break;

        case 8: // Lactic Acidosis
            alert("Lactic Acidosis! You lose two enzyme stations.");
            loseTwoStations(player);
            break;

        default:
            alert("Unknown special card effect.");
    }

    updateATP(player);
    console.log(`✅ Effect applied successfully to ${player.name}: ${specialCards[cardIndex].name}`);
}



function drawSpecialCard(p) {
    var p = player[turn]  // Get current player
    let cardIndex = Math.floor(Math.random() * specialCards.length); // Randomly pick a card
    let card = specialCards[cardIndex];

	if (!card) {
        console.error("Error: No special card found at index:", cardIndex);
        return;
    }

    alert(`You drew a Special Card: ${card.name}`);

    console.log(`🎴 Special Card Drawn: ${card.name}, Applying effect...`);
    
    applySpecialCardEffect(cardIndex, p);  // Apply the effect immediately
    updateATP(p);
}

function reverseLastEffect(player) {
    alert(player.name + " is reversing the last turn's effect!");
    // Implement logic for reversing last effect
}

function loseTwoStations(p) {
    if (player.properties.length < 2) {
        alert(player.name + " doesn't have enough stations to lose.");
        return;
    }

    // Remove two enzyme stations randomly
    for (let i = 0; i < 2; i++) {
        let lostStation = player.properties.pop();
        lostStation.owner = null;
        alert(player.name + " lost " + lostStation.name);
    }
}



function specialstationAction(specialstationIndex) {
	var p = player[turn]; // This is needed for reference in action() method.

	// $('#popupbackground').hide();
	// $('#popupwrap').hide();
	specialCards[specialstationIndex].action(p);

	updateATP();

	if (specialstationIndex !== 8 && !p.human) {
		p.AI.alertList = "";
		game.next();
	}
}
...
function addamount(amount, cause) {
	...
	addAlert(p.name + " received ATP" + amount + " from " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert(p.name + " lost ATP" + amount + " from " + cause + ".");
}

function gotojail() {
	var p = player[turn];
	addAlert(p.name + " was sent directly to Enzyme Inhibition Zone.");
	document.getElementById("landed").innerHTML = "You are in Enzyme Inhibition Zone.";
...
function payeachplayer(amount, cause) {
	...
	addAlert(p.name + " lost ATP" + total + " from " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert(p.name + " received ATP" + total + " from " + cause + ".");
}
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert(p.name + " lost ATP" + cost + " to Community Chest.");
		} else {
			addAlert(p.name + " lost ATP" + cost + " to Chance.");
		}
	}
...
function payfifty() {
	...
	if (p.money >= 50) {
        p.money -= 50;
        p.jail = false; // Player is released from jail
        p.jailroll = 0;
	document.getElementById("cell10").style.border = '1px solid black';
	document.getElementById("cell10").style.border = '2px solid ' + p.color;

	$("#landed").hide();
	doublecount = 0;
	p.position = 10;
	p.pay(50, 0);
	addAlert(p.name + " paid the 50 ATP fine to get out of enzyme inhibition zone.");
	updateATP(); 
	updatePosition();
	} else {
		alert("You don't have enough ATP to pay bail!");
...
function useJailCard() {
	...
	if (p.specialstationJailCard) {
		...
		specialCards.deck.splice(specialCards.index, 0, 0);

		specialCards.index++;

		if (specialCards.index >= specialCards.deck.length) {
			specialCards.index = 0;
		}
		...
		enzymeCards.deck.splice(enzymeCards.index, 0, 0);
	...
	addAlert(p.name + " used a \"Get Out of Enzyme Inhibition Zone Free\" card.");
	updateOwned();
	updatePosition();
}
...
$(document).ready(function() {
    window.hideStats = function() {
        console.log("🔻 hideStats() called! Closing popup...");
        alert("Closing stats popup!"); // Debugging
        $("#statsbackground").fadeOut(400, function () {
            $("#statswrap").hide();
        });
    };

    $("#statsclose").on("click", hideStats);
});


function showStats() {
    console.log("✅ showStats() function started.");
    ...
    for (var x = 1; x <= pcount; x++) {
        if (!player[x]) {
            console.error(`❌ Player ${x} not found!`);
            continue;
        }
        ...
        HTML += `<td class='statscell' id='statscell${x}' style='border: 2px solid ${p.color}'>
                    <div class='statsplayername'>${p.name}</div>`;

        for (var i = 0; i < 34; i++) {
            sq = square[i];

            if (sq.owner === x) {
                console.log("🏠 Processing property:", sq.name);

                mortgagetext = sq.mortgage ? "title='Mortgaged' style='color: grey;'" : "";
                housetext = "";
                ...
                HTML += `<tr>
                            <td class='statscellcolor' style='background: ${sq.color};' onmouseover='showdeed(${i});' onmouseout='hidedeed();'></td>
                            <td class='statscellname' ${mortgagetext}>${sq.name} ${housetext}</td>
                         </tr>`;
            }
        }
    ...
    console.log("✅ Stats updated in statstext div.");

    // Show using animation.
    $("#statsbackground").fadeIn(400, function() {
        console.log("🎉 Stats popup displayed.");
        $("#statswrap").show();
    });
}
...
function showdeed(property) {
	...
	console.log("Hovered Property Index:", property);
    console.log("Square Data:", square[property]);

	var deedElement = document.getElementById("deed");

    if (!deedElement) return;

    deedElement.style.display = "block";
    deedElement.style.top = event.clientY + 10 + "px";
    deedElement.style.left = event.clientX + 10 + "px";

    if (!square[property]) {
        console.error("Error: Property data missing for index", property);
        return;
    }
	console.log("📌 Station Data:", square[property]);
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			...
			document.getElementById("deed-price").textContent = "Purchase ATP " + sq.price;
			document.getElementById("deed-baserent").textContent = "Rent ATP " + sq.baserent;
			...
			$("#deed-special").show();
			document.getElementById("deed-special").style.backgroundColor = sq.color;
			document.getElementById("deed-special-name").textContent = sq.name;
			document.getElementById("deed-special-text").textContent = sq.pricetext;
			document.getElementById("deed-special-footer").textContent = "";
			// Clear price & rent for special and corner stations
			document.getElementById("deed-price").textContent = "";
			document.getElementById("deed-baserent").textContent = "";
			
		} else if (sq.groupNumber >= 9) {
				console.log("🔍 Handling groupNumber >= 9");
			
				$("#deed-normal").show();
				document.getElementById("deed-header").style.backgroundColor = sq.color || "gray";
				document.getElementById("deed-name").textContent = sq.name;
			
				// Use pricetext instead of price & baserent
...
function buy() {
	...
	if (p.money >= cost) {
		...
		popup("<p>" + p.name + ", you need ATP" + (property.price - p.money) + " more to buy " + property.name + ".</p>");
	}
}
...
function mortgage(index) {
	...
	document.getElementById("mortgagebutton").value = "Unmortgage for ATP" + unmortgagePrice;
	document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " for ATP" + unmortgagePrice + ".";

	addAlert(p.name + " mortgaged " + sq.name + " for ATP" + mortgagePrice + ".");
	updateOwned();
	updateATP();
...
function unmortgage(index) {
	...
	sq.mortgage = false;
	document.getElementById("mortgagebutton").value = "Mortgage for ATP" + mortgagePrice;
	document.getElementById("mortgagebutton").title = "Mortgage " + sq.name + " for ATP" + mortgagePrice + ".";

	addAlert(p.name + " unmortgaged " + sq.name + " for ATP" + unmortgagePrice + ".");
	updateOwned();
	return true;
}
...
function land(increasedRent) {
 ...
 if (p.position === 12) {
		...
		addAlert(p.name + " paid ATP" + rent + " rent to " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "You landed on " + s.name + ". " + player[s.owner].name + " collected ATP" + rent + " rent.";
	...
	// Go to Enzyme Inhibition Zone. Go directly to Enzyme Inhibition Zone. Do not pass GO. Do not collect $200.
	if (p.position === 30) {
		updateATP();
		updatePosition();

		if (p.human) {
			popup("<div>Go to Enzyme Inhibition Zone. Go directly to Enzyme Inhibition Zone. Do not pass GO. Do not collect $200.</div>", gotojail);
		} else {
			gotojail();
		}
	...
	if (!p.human) {
		popup(p.AI.alertList, landspecialstation);
		p.AI.alertList = "";
	} else {
		landspecialstation();
	}
...
function roll() {
	...
	if (die1 == die2 && !p.jail) {
		...
		// If player rolls doubles three times in a row, send him to Enzyme Inhibition Zone
			...
			if (p.human) {
				popup("You rolled doubles three times in a row. Go to Enzyme Inhibition Zone.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " rolled doubles to get out of Enzyme Inhibition Zone.");
...
function play() {
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "You are in Enzyme Inhibition Zone.<input type='button' title='Pay $50 fine to get out of Enzyme Inhibition Zone immediately.' value='Pay $50 fine' onclick='payfifty();' />";

		if (p.specialstationJailCard || p.chanceJailCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Use &quot;Get Out of Enzyme Inhibition Zone Free&quot; card.' onclick='useJailCard();' value='Use Card' />";
		}

		document.getElementById("nextbutton").title = "Roll the dice. If you throw doubles, you will get out of Enzyme Inhibition Zone.";

		if (p.jailroll === 0)
			addAlert("This is " + p.name + "'s first turn in Enzyme Inhibition Zone.");
		else if (p.jailroll === 1)
			addAlert("This is " + p.name + "'s second turn in Enzyme Inhibition Zone.");
			...
			addAlert("This is " + p.name + "'s third turn in Enzyme Inhibition Zone.");
		}
	...
	if (turn === 1) { // This means a full round has passed
		game.currentRound++;
		if (game.currentRound >= game.roundLimit) {
			alert("Game Over! The round limit has been reached.");
			endGame();
			return;
		}
	}
...
function endGame() { 
    // Filter out "the bank" and bankrupt players
    let validPlayers = player.filter(p => p.name !== "the bank" && p.money > 0);

    if (validPlayers.length === 0) {
        alert("Game Over! No players have ATP left. The bank wins by default.");
    } else {
        // Find the player with the highest ATP balance
        let winner = validPlayers.reduce((prev, curr) => (curr.atp > prev.atp ? curr : prev), validPlayers[0]);
        alert(`Game Over! ${winner.name} wins with ATP ${winner.money}!`);
    }
...
function setup() {
	var roundLimit = parseInt(document.getElementById("roundLimitSelect").value, 10); 
...
window.onload = function() {
	...
	for (var i = 0; i < 9; i++) {
		specialCards.deck[i] = i;
	}

    for (var i = 0; i < 36; i++) {
    enzymeCards.deck[i] = i;
    }

    for (var i = 0; i < 20; i++) {
        diseaseCards.deck[i] = i;
        }

	// Shuffle enzyme, special and disease decks.
	enzymeCards.deck.sort(function() {return Math.random() - 0.5;});
	specialCards.deck.sort(function() {return Math.random() - 0.5;});
    diseaseCards.deck.sort(function() {return Math.random() - 0.5;});
	...
	document.getElementById("enlarge0token").innerHTML += '<img src="images/start.png" height="40" width="136" alt="" />';
	document.getElementById("enlarge20price").innerHTML += "<img src='images/free-oxidation-station.png' height='80' width='72' alt='' style='position: relative; top: -20px;' />";
	...
	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Enzyme Inhibition Zone</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/inhibition-zone.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";

	document.getElementById("enlarge40name").innerHTML = "Enzyme Inhibition Zone";
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.55)) {
				popup("<p>You need ATP" + (Math.round(s.price * 0.55) - player[s.owner].money) + " more to unmortgage " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", are you sure you want to unmortgage " + s.name + " for ATP" + Math.round(s.price * 0.55) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Yes/No");
			}
		} else {
			popup("<p>" + player[s.owner].name + ", are you sure you want to mortgage " + s.name + " for ATP" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>You need ATP" + (s.houseprice - player[s.owner].money) + " more to buy a hotel for " + s.name + ".</p>");
				return;
			} else {
				popup("<p>You need ATP" + (s.houseprice - player[s.owner].money) + " more to buy a house for " + s.name + ".</p>");
				return;
			}
		}
	...
	document.getElementById("viewstats").addEventListener("click", function() {
		console.log("View Stats button clicked!"); // Debugging message
		alert("View Stats button clicked!"); // Extra confirmation
		showStats();
	});
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".cell").forEach(cell => {
        cell.addEventListener("mouseover", function () {
            let propertyIndex = this.id.replace("cell", ""); // Extract index
            showdeed(propertyIndex);
        });
