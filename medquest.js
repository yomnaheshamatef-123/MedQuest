	...
	this.bankruptcyUnmortgage = function() {
		...
		for (var i = 0; i < 34; i++) {
			...
			if (sq.owner == p.index && sq.mortgage) {
				...
				HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Unmortgage " + sq.name + " for ATP" + price + ".' onclick='if (" + price + " <= player[" + p.creditor + "].money) {player[" + p.creditor + "].pay(" + price + ", 0); square[" + i + "].mortgage = false; addAlert(\"" + player[p.creditor].name + " unmortgaged " + sq.name + " for ATP" + price + ".\");} this.parentElement.parentElement.style.display = \"none\";'>Unmortgage " + sq.name + " (ATP" + price + ")</a></td></tr>";
...
function landspecialstation() {
	...
	// Chance
		...
		|| p.position === 15 || p.position === 16 || p.position === 18 
        || p.position === 21 || p.position === 22 || p.position === 24 
        || p.position === 26 || p.position === 28 || p.position === 29 
...
function payToPreviousPlayer(player, amount) {
    let previousPlayer = game.players[(game.turn - 1 + game.players.length) % game.players.length];
    player.atp -= amount;
...
function payToNextPlayer(player, amount) {
    let nextPlayer = game.players[(game.turn + 1) % game.players.length];
    player.atp -= amount;
...
function rollDiceAgain(player, times) {
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (p.specialstationJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Get Out of Enzyme Inhibition Zone Free Card</td></tr>";
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>You landed on <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Buy (ATP" + s.price + ")' title='Buy " + s.name + " for " + s.pricetext + ".'/></div>";
		}
